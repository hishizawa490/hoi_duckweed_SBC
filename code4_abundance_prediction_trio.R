...
FLMboot<-function(i){
  ...
  if(i==0){sample<-data %>% filter(nspecies <=3)}
  ...
  for(n in 4:7){
    for(sys in unique(filter(preddata,nspecies==n)$system)){
      ...
      sub<-subsystem_all(sys,3,3)
      for(j in 1:n){
        constant[j,1]<-mean(as.matrix(filter(preddata,strain==mem[j],system %in% sub)$cfu))
        for(k in seq(1,n)[seq(1,n)!=j]){
          weight[k,j]<-(-0.5*(n-2)*(n-3)/choose(n-1,n-3))*mean(filter(int,spj==mem[j],spi==mem[k],system %in% sub)$Cij)
        }
      }
...
write.csv(result,"data6_abundance_prediction_trio.csv",row.names=FALSE)