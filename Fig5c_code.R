...
g<-list()
for(i in 1:7){
  g[[i]]<-ggplot(data=pred_tri %>% filter(strain==stnames[i]))+
    ...
    geom_text(data=metrics(pred_tri,group="strain") %>% filter(strain==stnames[i]),hjust=-0.1,vjust=1.4,size=8,
              aes(x=-Inf,y=+Inf,label=strain))+
    geom_text(data=metrics(pred_tri,group="strain") %>% filter(strain==stnames[i]),hjust=1.1,vjust=-0.6,size=7,
              aes(x=Inf,y=-Inf,label=paste("RMSPE ",scalefun(100*rmspe),"%",sep="")))+
    scale_fill_manual(values=brewer.pal(n=6,"YlGnBu")[3:6],guide="none")+
    scale_x_continuous(name=expression(paste("Predicted abundance (",{Log[10]}," cfu ",{mg^-1},")")),labels=scalefun,breaks=breaks_fun2,expand=c(0.18,0.1))+
    scale_y_continuous(name=expression(paste("Observed abundance ( ",{Log[10]}," cfu ",{mg^-1},")")),labels=scalefun,breaks=breaks_fun2,expand=c(0.18,0.1))+
